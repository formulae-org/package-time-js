...
Time.createTime = function(year, month, day, hour, minute, second, millisecond, timeZone) {
...
Time.prepareTimeForm = function() {
	if (Time.timeForm === undefined) {
		...
		for (let i = 0, n = Time.monthTags.length; i < n; ++i) {
			option = document.createElement("option");
		...
		ok.addEventListener("click", () => {
			let time = Time.createTime(
				document.getElementById("yy").value,
				document.getElementById("mo").selectedIndex,
...
Time.setEditions = function() {
	...
	Formulae.addEdition(Time.messages.pathTime, null, Time.messages.leafOffsetTime,           () => Expression.multipleEdition ("Time.OffsetTime", 8, 0));
	...
	for (let i = 0; i < 12; ++i) Formulae.addEdition(
		...
		() => Expression.replacingEdition("Time.Gregorian.Month." + Time.monthTags[i])
	);
	
	{
		...
		for (let i = 0; i < 7; ++i) Formulae.addEdition(
			...
			() => Expression.replacingEdition("Time.Gregorian.WeekDay." + Time.weekDayTags[(i + offset) % 7])
		);
	}

	for (let i = 0; i < 2; ++i) Formulae.addEdition(
		Time.messages.pathMeridiem, null, Time.messages.labelsMeridiem[i],
		() => Expression.replacingEdition("Time.Gregorian.Meridiem." + Time.meridiemTags[i])
	);
	...
	[ "MonthName", "WeekDayName", "FormatTimeShort", "FormatTimeMedium", "FormatTimeLong" ].forEach(tag => Formulae.addEdition(
